/**
 * JuriTask — bitacora.js
 * Bitácora de actividades a partir de los correos **enviados**, en lenguaje
 * neutro, lista para copiar al aplicativo de la empresa.
 *
 * Se genera con Gemini a partir del hilo real del trámite en Gmail. Nada se
 * envía ni se registra automáticamente: siempre se muestra para revisar y
 * copiar.
 *
 * Este archivo se llamaba `borradores.js` y traía además los **borradores del
 * día**: un lote que recorría las tareas de requerimiento vencidas y dejaba un
 * borrador puesto en cada hilo de Gmail. Esa función se retiró junto con
 * Firestore como base de datos; la bitácora se quedó porque es independiente y
 * se usa por su cuenta. Ver [bitacora-envios.md](../docs/bitacora-envios.md).
 */

/** Copia al portapapeles con aviso. Lo usan los modales de esta pantalla. */
